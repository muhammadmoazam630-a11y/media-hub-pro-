...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      rawOutput = execSync(
        `"${YTDLP_PATH}" --ignore-config --dump-json --flat-playlist --no-warnings --impersonate "chrome-124" --extractor-args "youtube:player_client=tv" --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36" --ignore-errors ${cookiesArg} ${proxyArg} ${JSON.stringify(url)}`,

...
export async function POST(request: NextRequest) {
  try {
    ...
    const output = execSync(
      `"${YTDLP_PATH}" --dump-json --flat-playlist --no-warnings --no-check-certificates --impersonate "chrome-125" --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36" --ignore-errors ${cookiesArg} ${JSON.stringify(url)}`,

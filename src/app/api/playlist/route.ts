...
export async function POST(request: NextRequest) {
  try {
    ...
    const output = execSync(
      `"${YTDLP_PATH}" --dump-json --flat-playlist --no-warnings --no-check-certificates --user-agent "Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Mobile Safari/537.36" --extractor-args "youtube:player_client=android;player_skip=webpage,configs" --ignore-errors ${cookiesArg} ${authArgs} ${JSON.stringify(url)}`,

...
export async function POST(request: NextRequest) {
  try {
    ...
    const output = execSync(
      `"${YTDLP_PATH}" --dump-json --flat-playlist --no-warnings --no-check-certificates --user-agent "${userAgent}" --extractor-args "${extractorArg}" --ignore-errors ${cookiesArg} ${JSON.stringify(url)}`,
